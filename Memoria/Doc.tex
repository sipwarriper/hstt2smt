...
%     Time, Name, Day, TimeGroups, TimeGroup, Resource, ResourceGroup, ResourceType, Event, Duration, Course, Role, 
%     AssignTimeConstraint, SplitEventsConstrain,DistributeSplitEventsConstraint, PreferTimesConstraint, SpreadEventsConstraint, AvoidClashesConstraint, AvoidUnavailableTimesConstraint, LimitIdleTimesConstraint, ClusterBusyTimesConstraint,
%     AppliesTio, Minimum, Maximum, Required, Weight, CostFunction, Week, Day, }
% }
  % \lstset{frame=tb,
  % language=Matlab,
  % aboveskip=3mm,
  ...
  \begin{itemize}
    \item Aprofundir sobre el problema treballat en sí i sobre els problemes de satisfacció de restriccions en general i les tècniques que s'utilitzen per resoldre'ls, com ara, SAT i les seves diverses extensions.
...
% afegits, suprimits o canviats d’ordre respecte del que descriu aquest document.
  ...
  Degut a que tot el software utilitzat és gratuït, lliure i de codi obert, i que el codi que el grup de recerca m'ha cedit també ha estat gratuït, no s'ha requerit de cap inversió ni despesa econòmica. 
  ...
  Tot i que, com s'ha explicat anteriorment, el desenvolupament d'aquest treball no té cost, s'ha decidit fer una simulació del que costaria contractar un programador per fer el treball. En la següent taula el cost:
