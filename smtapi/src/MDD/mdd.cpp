...
const vector<literal> & MDD::getSelectors() const{
	return selectors;
}
...
int MDD::getLayerWidth() {
	...
	for(int i = 0; i < id+1; i++)
		visited[i]=false;
 	list<MDD *> q;
	list<MDD *> * layers = new list<MDD *>[getVarDepth()];
	q.push_back(this);
	visited[getId()]=true;
...
pair<literal,MDD*> MDD::getSelectorAndChild(int idx) const{
	return pair<literal,MDD*> (selectors[idx],children[idx]);
}
...
ostream & operator<<(ostream & s, const MDD & m){
...
void MDD::print(ostream & s, bool *printed) const{
	if(!printed[id]){
		printed[id] = true;
		if(istrivialtrue)
			s << "id:1, vardepth:" << vardepth << ", realdepth:" << realdepth  << endl;
		else if(istrivialfalse)
			s << "id:0, vardepth:" << vardepth << ", realdepth:" << realdepth  << endl;
		else{
			s << "id:" << id << ", vardepth:" << vardepth << ", realdepth:" << realdepth << endl;
			for(int i = 0; i < selectors.size(); i++)
				s << "sel " << ((selectors[i].sign ? "" : "-") + selectors[i].v.id) 
				<< ":" << children[i]->getId() << endl; 
			s << "else:" << elsechild->getId() << endl;
