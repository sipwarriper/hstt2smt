...
AMOPBBDDBuilder::AMOPBBDDBuilder(const vector<vector<int> > &Q,const vector<vector<literal> > &X, int K, bool longedges)
	: MDDBuilder(){
	this->Q=Q;
	this->X=X;
	this->K=K;
...
AMOPBBDDBuilder::AMOPBBDDBuilder(const vector<int> &Q,const vector<literal> &X, int K, bool longedges)
	: MDDBuilder(){

	for(int q : Q)
		this->Q.push_back(vector<int>(1,q));

	for(literal x : X)
 		this->X.push_back(vector<literal>(1,x));
...
AMOPBBDDBuilder::~AMOPBBDDBuilder(){
  for(const vector<MDD *> & layer : L_BDDs)
	for(MDD * m : layer)
		delete m;
}
...
R_B AMOPBBDDBuilder::BDDConstruction(int global_l, int local_l, int l, int i_k) {
	...
	if(rb_new.bdd==NULL){
		...
		int B = max(inf_sum(rbtrue.B,q),rbfalse.B);
		int Y = min(inf_sum(rbtrue.Y,q),rbfalse.Y);
...
int AMOPBBDDBuilder::getmax(const vector<int> & v){
 int max = INT_MIN;
 for(int i : v)
  if(i > max)
    max = i;
...
int AMOPBBDDBuilder::getmax(const vector<int> & v, int init){
 int max = INT_MIN;
...
bool AMOPBBDDBuilder::hasInconsistentPath(const list<pair<literal,literal> > & mutexes) const {
	for(const pair<literal,literal> & mutex : mutexes)
...
void AMOPBBDDBuilder::createGraphviz(ostream & os, vector<vector<int> > * labels) const{
	if(root == NULL)
		return;
...
void AMOPBBDDBuilder::createGraphviz(MDD * mdd, ostream & os, bool * visited,int * global_idx,int * local_idx) const{
	...
	if(mdd->isTrueMDD())
	  ...
	  //os << " <= [" << B << "," << Y
		...
		for(int i = 0; i < mdd->getNSelectors(); i++){
			...
			os << "n_" << mdd->getId() << " -> n_" << child->getId() << " [label=\"" << Q[glob][loc] << "\"];" << endl;
		}
