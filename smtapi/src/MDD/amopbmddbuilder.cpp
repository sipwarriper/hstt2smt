...
AMOPBMDDBuilder::AMOPBMDDBuilder(const vector<vector<int> > &Q,const vector<vector<literal> > &X, int K, bool longedges)
	: MDDBuilder(){
	this->Q=Q;
	this->X=X;
	this->K=K;
...
AMOPBMDDBuilder::AMOPBMDDBuilder(const vector<int> &Q,const vector<literal> &X, int K, bool longedges)
	: MDDBuilder(){
...
AMOPBMDDBuilder::~AMOPBMDDBuilder(){
  for(const vector<MDD *> & layer : L_MDDs)
    for(MDD * m : layer)
      delete m;
}
...
int AMOPBMDDBuilder::getmax(const vector<int> & v){
	int max = INT_MIN;
	for(int i : v)
		if(i > max)
			max = i;
...
void AMOPBMDDBuilder::createGraphviz(ostream & os, vector<vector<int> > * labels) const{
...
void AMOPBMDDBuilder::createGraphviz(MDD * mdd, ostream & os, bool * visited) const{
	...
	if(mdd->isTrueMDD())
		...
		for(int i = 0; i < mdd->getNSelectors(); i++){
			...
			os << "n_" << mdd->getId() << " -> n_" << child->getId() << " [label=\"" << Q[l][i] << "\"];" << endl;
		}
