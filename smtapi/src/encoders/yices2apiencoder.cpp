...
bool Yices2APIEncoder::assertAndCheck(int lb, int ub, vector<literal> * assumptions){
...
term_t Yices2APIEncoder::getTerm(const literal & l, const vector<term_t> & boolvars, const vector<term_t> & intvars){
	term_t term;
	if(l.arith){
		...
		if(l.v.id<=0 || l.v.id > workingFormula.f->getNBoolVars()){
			cerr << "Error: asserted undefined Boolean variable: " << l.v.id << endl;
...
term_t Yices2APIEncoder::sumterm(const intsum & s, const vector<term_t> & intvars){
	if(s.v.empty())
		return yices_zero();
	else{
		vector<term_t> summands;
