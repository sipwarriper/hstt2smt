...
int Encoder::getObjective() const{
	if(enc==NULL){
		cerr << "Tried to access the objective value of a non-solved formula" << endl;
		exit(SOLVING_ERROR);
	}
...
bool Encoder::optimize(int lb, int ub){
	cerr << "The selected encoder does not support optimization" << endl;
	exit(UNSUPPORTEDFUNC_ERROR);
	return false;
}

bool Encoder::checkSATAssuming(int lb, int ub){
	cerr << "The selected encoder does not support checks with assumptions" << endl;
	exit(UNSUPPORTEDFUNC_ERROR);
	return false;
}
...
void Encoder::narrowBounds(int lb, int ub){
	cerr << "The selected encoder does not support narrowing bounds" << endl;
	exit(UNSUPPORTEDFUNC_ERROR);
}
