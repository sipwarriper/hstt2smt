...
SMTLIB2FileEncoder::SMTLIB2FileEncoder(Encoding * enc, const string & solver) : FileEncoder(enc){
	logic = "QF_LIA";
...
bool SMTLIB2FileEncoder::checkSAT(int lb, int ub){
   ...
   std::shared_ptr<FILE> pipe(popen(sentence.c_str(),"r"), pclose);
	...
	if(sat && produceModels()){
		vector<int> imodel(workingFormula.f->getNIntVars()+1);
		vector<bool> bmodel(workingFormula.f->getNBoolVars()+1);

		if(results.size() < workingFormula.f->getNIntVars() + workingFormula.f->getNBoolVars() + 1){
			cerr << "Error retrieving the solution from the SMT solver";
			exit(SOLVING_ERROR);
		}
...
void SMTLIB2FileEncoder::createFile(ostream & os, SMTFormula * f) const{

	if(produceModels())
		os << "(set-option :produce-models true)" << endl;

	os << "(set-logic " << logic << ")" << endl;
	for(int i = 1; i <= f->getNIntVars();i++)
		if(f->isDeclareVar(i))
			os << "(declare-fun " << ivn(f->getIntVarNames()[i]) << "() Int)" << endl;
	for(int i = 1; i <= f->getNBoolVars();i++)
			os << "(declare-fun " << bvn(f->getBoolVarNames()[i]) << "() Bool)" << endl;
	...
	if(solver=="optimathsat")
		os << "(get-objectives)" << endl;

	os << "(exit)" << endl;
}

inline string SMTLIB2FileEncoder::ivn(const string &s) const{
	return "i_"+s;
}

inline string SMTLIB2FileEncoder::bvn(const string &s) const{
	return "b_"+s;
}

void SMTLIB2FileEncoder::pclause(SMTFormula * f, const clause & c, ostream & os) const{
	if(c.v.size()==0)
		os << " false";
	else if(c.v.size()==1){
		pliteral(f,c.v[0],os);
...
void SMTLIB2FileEncoder::pliteral(SMTFormula * f, const literal & l, ostream & os) const{
	if(!l.sign)
		os << " (not";
	if(l.arith){
		if(l.cmp.eq)
...
void SMTLIB2FileEncoder::psum(SMTFormula * f, const intsum & s,ostream & os) const{
	if(s.v.empty())
			os << " 0";
	else if(s.v.size()==1){
		os << " ";
...
void SMTLIB2FileEncoder::pprod(SMTFormula * f, const intprod & p, ostream & os) const{
	if(p.varid<=0 || p.varid>f->getNIntVars()){
		cerr << "Error: asserted undefined Int variable"<< endl;
		exit(UNDEFINEDVARIABLE_ERROR);
	}
	if(p.coef==1)
