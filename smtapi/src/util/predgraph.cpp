...
PredGraph::PredGraph(int n)
{
  ...
  for(int i = 0; i < n; i++){
    ind[i]=i;
    coefficients[i] = new vector<int>();
  }
}

PredGraph::~PredGraph()
{
    for(vector<int> * v: coefficients)
      if(v!=NULL)
	delete v;
}
...
{
    //cout << "JOIN: " << a << "," << b << endl;
    vector<int> * first = coefficients[ind[a]];
    vector<int> * second = coefficients[b];    
...
void PredGraph::givenEdgesCover(list<pair<int,int> >edges){
  
  
  
}

void PredGraph::getSets(vector<set<int> > & groups,const vector<int> & mapping)
{
  map<int,set<int> > m;
