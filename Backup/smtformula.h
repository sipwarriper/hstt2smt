...
public:
    ...
    void addALOWithCheckVar(const vector<literal> & v, boolvar c);

	//Adds at-most-one constraint on the literals in 'v'
    void addAMO(const vector<literal> & v, AMOEncoding enc = AMO_QUAD);

    void addAMOWithCheckVar(const vector<literal> & v, boolvar c, AMOEncoding enc = AMO_QUAD);

	//Adds exactly-one constraint on the literals in 'v'
	void addEO(const vector<literal> & v, AMOEncoding enc = AMO_QUAD);

    void addEOWithCheckVar(const vector<literal> & v, boolvar c, AMOEncoding enc = AMO_QUAD);
